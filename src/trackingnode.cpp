...
tf::Point TrackingNode::get_target_point(void){
  ...
  tf::StampedTransform cam_to_marker_tf;
  ros::Time now = ros::Time::now();
  try {
    tf_listener_.waitForTransform("/cam","/tracking_target",now, ros::Duration(0.50));
    tf_listener_.lookupTransform("/cam", "/tracking_target", now, cam_to_marker_tf);
  } catch (tf::TransformException& ex) {
    ROS_ERROR("%s", ex.what());
    return cam_to_marker_tf.getOrigin();
  }
  return cam_to_marker_tf.getOrigin();
}
...
void TrackingNode::draw_arrow_rviz(tf::Vector3& endpoint) {
  ...
  cmd_vel_marker.header.frame_id = "/ardrone_base_link";
