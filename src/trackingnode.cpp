...
TrackingNode::TrackingNode() {
  ...
  m_transform_sub = nh.subscribe("/arsys_single_board/transform", 1,
                                 &TrackingNode::arsys_transform_callback, this);
  m_quad_vel_sub =
      nh.subscribe("/ardrone/odometry", 1, &TrackingNode::quad_OdomCallback,
                   this, ros::TransportHints().tcpNoDelay());
  m_arsys_pose_sub =
      nh.subscribe("/ar_single_board/pose", 1,
                   &TrackingNode::arsys_marker_pose_callback, this);

  m_cmd_vel_pub = nh.advertise<geometry_msgs::Twist>("/tracking/cmd_vel", 1);
  m_debug_pub = nh.advertise<std_msgs::Float64>("/pid/debug", 1);
  m_cmd_vel_marker_pub =
...
void TrackingNode::arsys_transform_callback(
    const geometry_msgs::TransformStamped& transformMsg) {
  //! This message comes from the ar_sys package.
  //! It has the transform of a single board related to the AR.Drone bottom
  //camera
  ...
  // We changed the digital_filter_change_rate variable from 0.5 to 0.75 and it
  // works better
...
tf::Transform& TrackingNode::m_tf_digital_filter(tf::Transform& dst,
