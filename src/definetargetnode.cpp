...
DefineTargetNode::DefineTargetNode()
{
    ...
    m_transform_sub = nh.subscribe ("/arsys_single_board/transform", 1, &DefineTargetNode::arsys_transform_callback, this);
    m_arsys_pose_sub = nh.subscribe("/ar_single_board/pose", 1, &DefineTargetNode::arsys_marker_pose_callback, this);

    m_cmd_vel_pub = nh.advertise<geometry_msgs::PoseStamped>("/tracking/target_pose", 1);
...
///Only for one marker
void DefineTargetNode::arsys_marker_pose_callback(const geometry_msgs::PoseStamped& marker_pose_msg){
...
void DefineTargetNode::arsys_transform_callback (const geometry_msgs::TransformStamped& transformMsg){
...
tf::Transform& DefineTargetNode::m_tf_digital_filter(tf::Transform &dst, const tf::Transform &src)
{
